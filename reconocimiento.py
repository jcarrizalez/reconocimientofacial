...
def Personas():
    with open('personas/db.json', 'r') as file:
        users = json.load(file)
        data = [user.get('nombre') for user in users]
    return data

def TuSiTuNo(EllosSi):        
    if EllosSi in Personas() and EllosSi not in yaReconocido:
        yaReconocido.append(EllosSi)
        print('Bienvenido {}'.format(EllosSi))
    elif EllosSi not in Personas():
...
while True:
    ...
    #    if area > 1 and area < 210000:
    #        cv2.drawContours(frame, [c], 0, (0, 0, 255), 1, cv2.LINE_AA)
   ...
   guardamos su posicion"""
    ...
    for i in range(len(faces)):
        ...
        #Si la prediccion es mayor a 100 no es un reconomiento con la exactitud suficiente
            ...
            cv2.putText(frame, 'Desconocido',(x-10, y-10), cv2.FONT_HERSHEY_PLAIN,1,(255, 255, 0))  
    ...
    if key == 27:
        ...
        break
    time.sleep(0.001)
#convert pote.jpeg -fuzz 20% -transparent black output.png